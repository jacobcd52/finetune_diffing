...
from transformers.models.mixtral.modeling_mixtral import (F, Callable, List, Optional, Tuple, Union, torch, nn, ACT2FN, Cache, GenerationMixin, FlashAttentionKwargs, CausalLMOutputWithPast, MoeCausalLMOutputWithPast, MoeModelOutputWithPast, ROPE_INIT_FUNCTIONS, dynamic_rope_update, ALL_ATTENTION_FUNCTIONS, PreTrainedModel, Unpack, add_start_docstrings, add_start_docstrings_to_model_forward, can_return_tuple, replace_return_docstrings, deprecate_kwarg, MixtralConfig, logger, __name__, _CONFIG_FOR_DOC, MixtralPreTrainedModel, MIXTRAL_INPUTS_DOCSTRING, MixtralModel, KwargsForCausalLM, MixtralForCausalLM)

@torch.compile(fullgraph = False, dynamic = True, options = torch_compile_options)
def MixtralBlockSparseTop2MLP_forward(self, hidden_states):
...
class MixtralBlockSparseTop2MLP(nn.Module):
    ...
    def forward(self, hidden_states):
        return MixtralBlockSparseTop2MLP_forward(self, hidden_states)


@torch.compile(fullgraph = True, dynamic = True, options = torch_compile_options)
def MixtralRMSNorm_forward(self, hidden_states):
...
class MixtralRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        return MixtralRMSNorm_forward(self, hidden_states)
...
class MixtralAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        return MixtralAttention_forward(self, hidden_states, position_embeddings, attention_mask, past_key_value, cache_position, **kwargs)
...
class MixtralRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        return MixtralRotaryEmbedding_forward(self, x, position_ids)
...
class MixtralForCausalLM(MixtralPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        return MixtralForCausalLM_forward(self, input_ids, attention_mask, position_ids, past_key_values, inputs_embeds, labels, use_cache, output_attentions, output_hidden_states, output_router_logits, cache_position, logits_to_keep, **kwargs)
